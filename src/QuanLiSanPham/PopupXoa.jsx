...
const PopupXoa = ({ show, onClose, item, onSave }) => {
  ...
  const handleXacNhanXoa = () => {
    fetch(
      `api/product-service/employee/product/delete-product?id=${item.productId}`,
      {
        method: "DELETE",
        headers: {
        ...
        <div className="space-y-4">
          <div>
            <label className="block text-blue-700">
              Bạn có muốn xóa sản phẩm: {item.name}
            </label>
          </div>
          {responseDelete && responseDelete.tag === "ErrorDeleteProduct" && (
